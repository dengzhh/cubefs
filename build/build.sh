...
build_server() {
    ...
    CGO_ENABLED=1 go build -gcflags="${MODFLAGS}" -ldflags="${LDFlags}" -o ${BuildBinPath}/cfs-server ${SrcPath}/cmd/*.go && echo "success" || echo "failed"
    popd >/dev/null
}

build_clustermgr() {
    CGO_ENABLED=1 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/blobstore ${SrcPath}/blobstore/cmd/clustermgr
}

build_blobnode() {
    CGO_ENABLED=1 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/blobstore ${SrcPath}/blobstore/cmd/blobnode
}

build_access() {
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/blobstore ${SrcPath}/blobstore/cmd/access
}

build_scheduler() {
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/blobstore ${SrcPath}/blobstore/cmd/scheduler
}

build_proxy() {
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/blobstore ${SrcPath}/blobstore/cmd/proxy
}

build_blobstore_cli() {
    CGO_ENABLED=1 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/blobstore/blobstore-cli ${SrcPath}/blobstore/cli/cli
}
...
build_client() {
    ...
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -ldflags="${LDFlags}" -o ${BuildBinPath}/cfs-client ${SrcPath}/client/*.go  && echo "success" || echo "failed"
    popd >/dev/null
}
...
build_cli() {
    ...
    CGO_ENABLED=1 go build -gcflags="${MODFLAGS}" -ldflags="${LDFlags}" -o ${BuildBinPath}/cfs-cli ${SrcPath}/cli/*.go  && echo "success" || echo "failed"
...
build_fsck() {
    ...
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -ldflags="${LDFlags}" -o ${BuildBinPath}/cfs-fsck ${SrcPath}/fsck/*.go  && echo "success" || echo "failed"
    popd >/dev/null
}
...
build_libsdkpre() {
    ...
    CGO_ENABLED=1 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -buildmode c-shared -o ${TargetFile} ${SrcPath}/libsdk/*.go && echo "success" || echo "failed"
    popd > /dev/null
}
...
build_libsdk() {
    ...
    pushd $SrcPath >/dev/null
    echo -n "build libsdk: libcfs.so       "
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/fdstore ${SrcPath}/fdstore/*.go  && echo "success" || echo "failed"
    popd >/dev/null
...
build_preload() {
    ...
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/cfs-preload ${SrcPath}/preload/*.go && echo "success" || echo "failed"
}

build_bcache(){
    pushd $SrcPath >/dev/null
    echo -n "build cfs-blockcache      "
    CGO_ENABLED=0 go build -gcflags="${MODFLAGS}" -gcflags=all=-trimpath=${SrcPath} -asmflags=all=-trimpath=${SrcPath} -ldflags="${LDFlags}" -o ${BuildBinPath}/cfs-bcache ${SrcPath}/blockcache/*.go  && echo "success" || echo "failed"
    popd >/dev/null
}
